...
| **ALB weighted forward (`/` only)** | **UAT:** **AWS Load Balancer Controller** splits **browser** traffic between **`web`** and **`web-canary`** by weight (**default 10%** canary). **Dev/QA/Prod** default **off** (no extra nginx). **API** paths unchanged—no duplicate Go services. |
