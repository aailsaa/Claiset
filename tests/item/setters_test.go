...
func TestSetColors(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if !reflect.DeepEqual(currItem.GetColors(), test.expected) {
			t.Errorf("\nTest %d failed: Expected colors: %v\nActual: %v", idx, cb.StringColors(test.expected), cb.StringColors(currItem.GetColors()))
		} else {
			t.Logf("\nTest %d passed: Expected/actual colors: %v", idx, cb.StringColors(currItem.GetColors()))
		}
	}
}
...
func TestSetSubcategory(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if currItem.GetSubcategory() != test.expected {
			t.Errorf("\nTest %d failed: Expected subcategory: %d\nActual: %d", idx, test.expected, currItem.GetSubcategory())
			continue
		} else {
			t.Logf("\nTest %d passed: Expected/actual subcategory: %d", idx, currItem.GetSubcategory())
		}
	}
}
...
func TestSetID(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if currItem.GetID() != test.expected {
			t.Errorf("\nTest %d failed: Expected ID: %d\nActual: %d", idx, test.expected, currItem.GetID())
		} else {
			t.Logf("\nTest %d passed: Expected/actual ID: %d", idx, currItem.GetID())
		}
	}
}
...
func TestSetDate(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if !currItem.GetDate().Equal(test.expected) {
			t.Errorf("\nTest %d failed: Expected date: %s\nActual: %s", idx, test.expected.String(), currItem.GetDate().String())
		} else {
			t.Logf("\nTest %d passed: Expected/actual date: %s", idx, currItem.GetDate().String())
		}
	}
}
...
func TestSetPrice(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if currItem.GetPrice() != test.expected {
			t.Errorf("\nTest %d failed: Expected price: %.2f\nActual: %.2f", idx, test.expected, currItem.GetPrice())
		} else {
			t.Logf("\nTest %d passed: Expected/actual price: %.2f", idx, currItem.GetPrice())
		}
	}
}
...
func TestSetWears(t *testing.T) {
	...
	for idx, test := range tests {
		...
		if currItem.GetWears() != test.expected {
			t.Errorf("\nTest %d failed: Expected wears: %d\nActual: %d", idx, test.expected, currItem.GetWears())
		} else {
			t.Logf("\nTest %d passed: Expected/actual wears: %d", idx, currItem.GetWears())
		}
	}
}
